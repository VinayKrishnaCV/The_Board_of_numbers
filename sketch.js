...
function draw() {
  ...
  if(particle!=null){
    ...
    if(particle.body.position.y>500){
      if(particle.body.position.x>12&&particle.body.position.x<82){
        score+=1000
        ...
        if(count>=5) gameState="end"
        particle=null
      }else if(particle.body.position.x>172&&particle.body.position.x<243){
        score+=150
        if(count>=5) gameState="end"
        particle=null
      }else if(particle.body.position.x>652&&particle.body.position.x<722){
      score+=100
      if(count>=5) gameState="end"
      particle=null
        ...
        if(count>=5) gameState="end"
        particle=null
      }else if((particle.body.position.x>333&&particle.body.position.x<402)||(particle.body.position.x>493&&particle.body.position.x<562)){
        score+=5
        if(count>=5) gameState="end"
        particle=null
      }else if(particle.body.position.x>412&&particle.body.position.x<483){
        score+=1
        if(count>=5) gameState="end"
        particle=null
      }
